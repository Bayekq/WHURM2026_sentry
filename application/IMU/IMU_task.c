/**
  ...
  *                                             2. 删除了大量旧代码
  *  V3.0.1     Sep-25-2025     Bayek           1. 增加了连续角度
  *
  @verbatim
  ==============================================================================
...
static void UpdateImuData(void)
{
    IMU_DATA.angle_last[AX_X] = IMU_DATA.angle[AX_X];
    IMU_DATA.angle_last[AX_Y] = IMU_DATA.angle[AX_Y];
    IMU_DATA.angle_last[AX_Z] = IMU_DATA.angle[AX_Z];
    ...
    IMU_DATA.total_angle[AX_X] = ContinuousAngle(IMU_DATA.angle[AX_X], &IMU_DATA.angle_last[AX_X], &IMU_DATA.round[AX_X]);
    IMU_DATA.total_angle[AX_Y] = ContinuousAngle(IMU_DATA.angle[AX_Y], &IMU_DATA.angle_last[AX_Y], &IMU_DATA.round[AX_Y]);
    IMU_DATA.total_angle[AX_Z] = ContinuousAngle(IMU_DATA.angle[AX_Z], &IMU_DATA.angle_last[AX_Z], &IMU_DATA.round[AX_Z]);
}
...
/**
  * @brief          获取连续角度
  * @param[in]      axis:轴id，可配合定义好的轴id宏使用
  * @retval         (rad) axis轴的连续角度值
  */
inline float GetImuTotalAngle(uint8_t axis) { return IMU_DATA.total_angle[axis]; }
/**
  * @brief          获取角速度
  * @param[in]      axis:轴id，可配合定义好的轴id宏使用
