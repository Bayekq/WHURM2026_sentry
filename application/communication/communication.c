/**
  ...
  *  V1.2.1     Nov-25-2025     Bayek           1. 新增C板类型、数据类型和API接口
  *
  @verbatim
  ==============================================================================
...
Data_Bigyaw_s Send_Data_Bigyaw;
Data_Aimyaw_s Send_Data_Aimyaw;

// receive data
Data_Test_s   Receive_Data_Test;
Data_Bigyaw_s Receive_Data_Bigyaw;
Data_Aimyaw_s Receive_Data_Aimyaw;
...
{
    UART2DataInit(Test);
    UART2DataInit(Bigyaw);
    UART2DataInit(Aimyaw);
}
...
void DataBigyawRenew()
{
    Send_Data_Bigyaw.time_stamp = HAL_GetTick();

    Send_Data_Bigyaw.data.yaw_motor_offline = true;
    memcpy(&Send_Data_Bigyaw.data.rc_ctrl, get_remote_control_point(), sizeof(RC_ctrl_t));
    Send_Data_Bigyaw.data.rc_offline = GetRcOffline();
    Send_Data_Bigyaw.data.init_judge = GetGimbalInitJudgeReturn();
    append_CRC16_check_sum((uint8_t *)(&Send_Data_Bigyaw), sizeof(Data_Bigyaw_s));
}

void DataAimyawRenew()
{
    Send_Data_Aimyaw.time_stamp = HAL_GetTick();

    Send_Data_Aimyaw.data.rc_offline = GetUartRcOffline();
    Send_Data_Aimyaw.data.auto_aim_judge = GetGimbalAutoAimJudgeReturn();
...
void Uart2DataSolve(uint8_t * frame)
{
    ...
    switch (frame_header.id) {
        ...
        case Uart2_Data_Bigyaw_ID: {
            memcpy(&Receive_Data_Bigyaw, frame, sizeof(Data_Bigyaw_s));
            LastReceiveTime.Data_Bigyaw = HAL_GetTick();

#if __CONTROL_LINK_RC == CL_RC_UART2
            const RC_ctrl_t * rc_ctrl = get_remote_control_point();
            memcpy((RC_ctrl_t *)rc_ctrl, &Receive_Data_Bigyaw.data.rc_ctrl, sizeof(RC_ctrl_t));
#endif
        } break;
        case Uart2_Data_Aimyaw_ID: {
            memcpy(&Receive_Data_Aimyaw, frame, sizeof(Data_Aimyaw_s));
            LastReceiveTime.Data_Aimyaw = HAL_GetTick();
        } break;
        default:
            break;
    }
...
#if __SELF_BOARD_ID == C_BOARD_DEFAULT
    Uart2CheckDurationAndSend(Test);
#elif __SELF_BOARD_ID == C_BOARD_OMNI_SENTRY_ROT_GIMBAL
    Uart2CheckDurationAndSend(Bigyaw);
#elif __SELF_BOARD_ID == C_BOARD_OMNI_SENTRY_AIM_GIMBAL
    Uart2CheckDurationAndSend(Aimyaw);
#endif
...
/*     GetUartRotGimbalInitJudge                                               */
/*     GetUartAimGimbalInitJudge                                               */
...
bool GetUartRcOffline(void)
{
    ...
    return Receive_Data_Bigyaw.data.rc_offline;
}
...
float GetUartGimbalYawMotorPos(void)
{
    ...
    return Receive_Data_Aimyaw.data.yaw_motor_offset;
}

bool GetUartRotGimbalInitJudge(void)
{
    if (GetUartOffline()) {
        return false;
    }
    return Receive_Data_Bigyaw.data.init_judge;
}

bool GetUartAimGimbalInitJudge(void)
{
    if (GetUartOffline()) {
        return false;
    }
    return Receive_Data_Aimyaw.data.init_judge;
}

bool GetUartAutoAimJudgeReturn(void)
{
    if (GetUartOffline()) {
        return false;
    }
