...
/**
 * @brief 连续角度计算，用于处理角度在-PI~PI之间的过圈问题
 * @author Bayekq
 * @param theta 当前角度
 * @param last_theta 上一角度
 * @param round_count 过圈计数
 * @return 连续角度
 */
float ContinuousAngle(float theta, float *last_theta, int *round_count)
{
    *round_count += MotorRoundCount(theta, *last_theta);
	*last_theta = theta;
    return theta += (*round_count) * 2 * PI;
}

/**
 * @brief 计算角度过圈次数，用于处理角度在-PI~PI之间的过圈问题
 * @author Bayekq
 * @param theta 当前角度
 * @param last_theta 上一角度
 * @return 过圈次数
 */
int MotorRoundCount(float theta, float last_theta)
{
